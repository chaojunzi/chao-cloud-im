...
import com.chao.cloud.im.domain.vo.LoginVO;
...
public class LoginController extends BaseController {
	...
	public String index(HttpSession session) {
		return isLogin(session) ? "layim/index" : "redirect:/login";
	}

	@GetMapping("/login")
	public String login(HttpSession session) {
		return isLogin(session) ? "redirect:/layim/index" : "redirect:/login/login.html";
	}
	...
	public Response<String> ajaxLogin(@Valid LoginVO vo, HttpSession session) {
		...
		// 根据登录名获取用户
		ImUser user = imUserService.getOne(Wrappers.<ImUser>lambdaQuery().eq(ImUser::getUserName, vo.getUserName()));
		...
		if (!ImConstant.Status.OK.equals(user.getStatus())) {
